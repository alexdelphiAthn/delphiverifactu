...
> **Compilación de los ejemplos:** son proyectos de consola autocontenidos. Ábrelos en RAD Studio o compílalos con `dcc32 <Programa>.dpr`. Los ejemplos `01`, `03` (firma), `04` (envío real) y la firma de `07`/`08` usan el almacén de certificados de **Windows**; el `05` usa `Vcl.Graphics`. La construcción de XML, huellas, URLs y el control de reloj (`06`) es multiplataforma; los ejemplos `07` y `08` funcionan también en modo demo (huella SHA-256, sin firma) si no indicas certificado.
...
- [ ] Verificación local de los ficheros NO VERI*Factu exportados (estructura, cadena de huellas y perfil de firma).
